...
url1 = 'http://dedicatedexcel.com/wp-content/uploads/2018/08/00109_UK-Football-Fixtures-2018-19-by-Dedicated-Excel.xlsx'

##Stores the downloaded csv file in a dataframe
initialStats = pd.read_csv(url, header=0)
##Reset index
...
df3 = initialFixtures[initialFixtures.League == 'EPL']
df4 = df3[requiredColumns2]
df5 = df3[requiredColumn]
df5['BTTS'] = '?'
##Data moved to csv file structures.
df4.to_csv("UpcomingFixtures.csv", index=False)
df5.to_csv("UpcomingBTTS.csv", index=False)

df6 = initialStats[requiredCoulumn3]
df6.to_csv("Stats.csv", index=False)
